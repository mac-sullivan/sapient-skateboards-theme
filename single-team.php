<?php
/**
 * Single Team Member page — /team/{slug}/
 */
get_header();

while ( have_posts() ) : the_post();
    $position = get_field( 'team_position' );
    $bio      = get_field( 'team_description' );
    $photo    = get_field( 'team_image' );
    $socials  = get_field( 'team_socials' );


    $social_icons = [
        'instagram' => '<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>',
        'tiktok'    => '<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1V9.01a6.27 6.27 0 00-.79-.05 6.34 6.34 0 00-6.34 6.34 6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.33-6.34V8.83a8.18 8.18 0 004.78 1.52V6.9a4.85 4.85 0 01-1.01-.21z"/></svg>',
        'twitter'   => '<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.737-8.835L1.254 2.25H8.08l4.259 5.63zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>',
        'youtube'   => '<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M23.495 6.205a3.007 3.007 0 00-2.088-2.088c-1.87-.501-9.396-.501-9.396-.501s-7.507-.01-9.396.501A3.007 3.007 0 00.527 6.205a31.247 31.247 0 00-.522 5.805 31.247 31.247 0 00.522 5.783 3.007 3.007 0 002.088 2.088c1.868.502 9.396.502 9.396.502s7.506 0 9.396-.502a3.007 3.007 0 002.088-2.088 31.247 31.247 0 00.5-5.783 31.247 31.247 0 00-.5-5.805zM9.609 15.601V8.408l6.264 3.602z"/></svg>',
        'facebook'  => '<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>',
        'website'   => '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="square"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 010 20M12 2a15.3 15.3 0 000 20"/></svg>',
    ];
?>

<main id="main-content" class="single-team-page">

  <!-- ── Profile body ───────────────────────────────────────── -->
  <div class="single-team-body">
    <div class="container single-team-inner">

      <!-- Photo -->
      <div class="single-team-photo-col">
        <?php if ( $photo ) : ?>
          <div class="single-team-photo">
            <img src="<?php echo esc_url( $photo['url'] ); ?>"
                 alt="<?php echo esc_attr( $photo['alt'] ?: get_the_title() ); ?>">
          </div>
        <?php else : ?>
          <div class="single-team-photo single-team-photo--empty">
            <img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/images/skull.svg"
                 alt="<?php the_title(); ?>" class="single-team-skull-placeholder">
          </div>
        <?php endif; ?>

        <?php if ( $socials ) : ?>
          <ul class="single-team-socials">
            <?php foreach ( $socials as $link ) :
              $platform = $link['platform'] ?? '';
              $url      = $link['url'] ?? '';
              if ( ! $url ) continue;
              $icon = $social_icons[ $platform ] ?? $social_icons['website'];
            ?>
              <li>
                <a href="<?php echo esc_url( $url ); ?>" target="_blank" rel="noopener"
                   aria-label="<?php echo esc_attr( ucfirst( $platform ) ); ?>">
                  <?php echo $icon; ?>
                  <span><?php echo esc_html( ucfirst( $platform ) ); ?></span>
                </a>
              </li>
            <?php endforeach; ?>
          </ul>
        <?php endif; ?>
      </div>

      <!-- Info -->
      <div class="single-team-info-col">
        <div class="single-team-name-block">
          <h1 class="single-team-name"><?php the_title(); ?></h1>
          <?php if ( $position ) : ?>
            <span class="single-team-position"><?php echo esc_html( $position ); ?></span>
          <?php endif; ?>
        </div>

        <?php if ( $bio ) : ?>
          <div class="single-team-bio wysiwyg">
            <?php echo wp_kses_post( $bio ); ?>
          </div>
        <?php endif; ?>

        <a href="<?php echo esc_url( home_url( '/about/crew' ) ); ?>" class="single-team-back-btn">← Back to Crew</a>
      </div>

    </div>
  </div>

</main>

<?php endwhile; ?>
<?php get_footer(); ?>
